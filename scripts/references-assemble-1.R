...
prefixes.list <- c("12s.12S-V5", "12s.bylemans-AcMDB", "12s.bylemans-MarVer1",
                   "12s.evans-Ac", "12s.evans-Am", "12s.kelly", "12s.miya", 
                   "12s.miya-E", "12s.miya-U", "12s.riaz", "12s.riaz-1", 
                   "12s.taberlet", "12s.taberlet-Elas02", "12s.taberlet-Tele02", 
                   "12s.valentini", "12s.valentini-1", "16s.berry", 
                   "16s.berry-1", "16s.bylemans",  
                   "16s.evans-Ac", "16s.evans-Ve", "16s.kitano", 
                   "16s.kitano-1", "16s.mcinnes", "16s.palumbi", 
                   "16s.shaw", "16s.vences", "atp6.mieel", 
                   "coi.collins-mid", "coi.collins-short", "coi.folmer", 
                   "coi.geller", "coi.ivanova-FF2s", "coi.ivanova-VF1d", 
                   "coi.ivanova-Vf1i", "coi.lerayxt", "coi.meusnier", 
                   "coi.meyer", "coi.ward", "coi.ward-1", 
                   "coi.ward-2", "coi.weigt", "coi.west-F1", 
                   "coi.west-F2", "cytb.burgener", "cytb.hanfling", 
                   "cytb.minamoto", "cytb.minamoto-1", "cytb.miya", 
                   "cytb.miyanishida", "cytb.palumbi", "cytb.sevilla", 
                   "cytb.thomsen-1", "cytb.thomsen-2", "cytb.thomsen-2b", 
                   "cytb.thomsen-3", "cytb.thomsen-CB", "cytb.tsuji"
                   )
# split the input
...
if(opt$metabarcode == "all") {
    prefixes.all <- c("12s.12S-V5.noprimers", "12s.bylemans-AcMDB.noprimers", "12s.bylemans-MarVer1.noprimers",
                      "12s.evans-Ac.noprimers", "12s.evans-Am.noprimers", "12s.kelly.noprimers", "12s.miya.noprimers", 
                      "12s.miya-E.noprimers", "12s.miya-U.noprimers", "12s.riaz.noprimers", "12s.riaz-1.noprimers", 
                      "12s.taberlet.noprimers", "12s.taberlet-Elas02.noprimers", "12s.taberlet-Tele02.noprimers", 
                      "12s.valentini.noprimers", "12s.valentini-1.noprimers", "16s.berry.noprimers", 
                      "16s.berry-1.noprimers", "16s.bylemans.noprimers",  
                      "16s.evans-Ac.noprimers", "16s.evans-Ve.noprimers", "16s.kitano.noprimers", 
                      "16s.kitano-1.noprimers", "16s.mcinnes.noprimers", "16s.palumbi.noprimers", 
                      "16s.shaw.noprimers", "16s.vences.noprimers", "atp6.mieel.noprimers", 
                      "coi.collins-mid.noprimers", "coi.collins-short.noprimers", "coi.folmer.noprimers", 
                      "coi.geller.noprimers", "coi.ivanova-FF2s.noprimers", "coi.ivanova-VF1d.noprimers", 
                      "coi.ivanova-Vf1i.noprimers", "coi.lerayxt.noprimers", "coi.meusnier.noprimers", 
                      "coi.meyer.noprimers", "coi.ward.noprimers", "coi.ward-1.noprimers", 
                      "coi.ward-2.noprimers", "coi.weigt.noprimers", "coi.west-F1.noprimers", 
                      "coi.west-F2.noprimers", "cytb.burgener.noprimers", "cytb.hanfling.noprimers", 
                      "cytb.minamoto.noprimers", "cytb.minamoto-1.noprimers", "cytb.miya.noprimers", 
                      "cytb.miyanishida.noprimers", "cytb.palumbi.noprimers", "cytb.sevilla.noprimers", 
                      "cytb.thomsen-1.noprimers", "cytb.thomsen-2.noprimers", "cytb.thomsen-2b.noprimers", 
                      "cytb.thomsen-3.noprimers", "cytb.thomsen-CB.noprimers", "cytb.tsuji.noprimers"
                      )
...
# Function to run HMMER for a batch of prefixes
run_hmmer_batch <- function(prefixes_batch, batch_index) {
  writeLines("\nExtracting metabarcode fragments with HMMER (may take several minutes) ...")
  dat.frag.all <- lapply(prefixes_batch, function(x) run_hmmer3(dir="temp",
                                                                infile="mtdna-dump.fas",
                                                                prefix=x,
                                                                evalue="10",
                                                                coords="env"))
  writeLines("\nDone")
  
  saveRDS(dat.frag.all, file=paste0("temp/dat_frag_batch_", batch_index, ".rds"))
}

# Split prefixes into smaller batches
batch_size <- 3  # Adjust batch size as needed
prefixes_batches <- split(prefixes.all, ceiling(seq_along(prefixes.all) / batch_size))

for (batch_index in seq_along(prefixes_batches)) {
  run_hmmer_batch(prefixes_batches[[batch_index]], batch_index)
}
# Run HMMER for each batch and combine results
dat.frag.all <- list()

for (batch_index in seq_along(prefixes_batches)) {
  dat.frag.batch <- readRDS(file=paste0("temp/dat_frag_batch_", batch_index, ".rds"))
  dat.frag.all <- c(dat.frag.all, dat.frag.batch)
}
